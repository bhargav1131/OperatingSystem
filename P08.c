/*
Question:Write a program to show how two processes can talk to each other using wait() and signal()
operations applied on semaphore.

Implementation: Program creates two child processes: one to increment the value of a shared variable and second to decrement the value of the shared variable. Both the threads make use of semaphore variable so that only one of the threads is executing in its critical section 
